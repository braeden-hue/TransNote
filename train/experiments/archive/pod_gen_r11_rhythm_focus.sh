#!/bin/bash
# 밀집 리듬(8분/16분 런) 집중 데이터 1500장 생성 -- pod_gen_rhythm_focus.sh(2026-07-31)
# 재사용, N_SHARDS만 이번 세션 pod 실측(cpu.max=13.6코어)에 맞게 20->6으로 낮춤
# (POD_TRAINING_CHECKLIST.md 5번 항목: nproc이 아니라 cpu.max 코어 수의 절반 기준).
# 그 외 생성 파라미터(--dotted8-bias 20.0 --eighth-run-prob 0.30 --sixteenth-run-prob 0.35
# 등)는 원본과 동일 -- sonata_30_44 등 held-out 실측에서 확인된 "밀집 16분음표 런" 패턴을
# 직접 타깃한다.
set -uo pipefail
cd /workspace/round3train
MUSESCORE=/workspace/musescore/squashfs-root/AppRun
LOG=/workspace/gen_r11_rhythm_focus.log
STATUS=/workspace/gen_r11_rhythm_focus_status.txt
exec >> "$LOG" 2>&1
echo "=== $(date) 밀집 리듬 집중 데이터 생성 시작(r11) ==="
: > "$STATUS"
...
COMMON_ARGS=(--min-measures 2 --max-measures 6 --chord-prob 0.10
             ...
             --dotted8-bias 20.0 --eighth-run-prob 0.30 --sixteenth-run-prob 0.35
             --eighth-run-prob-2-4 0.15 --sixteenth-run-prob-2-4 0.45
  ...
  while [ "$existing" -lt "$target" ] && [ "$round" -lt 3 ]; do
    ...
    for i in $(seq 0 $((n_shards - 1))); do
      ...
      xvfb-run -a python3 generate_scores.py \
        ...
        > "/workspace/gen_r11_${stage_name}_r${round}_${i}.log" 2>&1 &
      pids+=($!)
    done
    local fail=0
...
POOL_DIR=data/r11_rhythm_focus_pool
mkdir -p "$POOL_DIR"

gen_stage "rhythm_single" "$POOL_DIR" 375  12000001 1100001 1
gen_stage "rhythm_grand"  "$POOL_DIR" 1125 13000001 1110001 0 "${GRAND_ONLY_ARGS[@]}"

TOTAL_N=$(find "$POOL_DIR" -maxdepth 1 -name '*.png' | wc -l)
echo "=== $(date) 밀집 리듬 집중 데이터 생성 종료: 총 ${TOTAL_N}/1500장 ==="
