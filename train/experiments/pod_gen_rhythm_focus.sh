#!/bin/bash
# 2단계(밀집 리듬 집중) 데이터 1500장 생성 (2026-07-31).
# 1단계(화음 집중)와 같은 배경(과잉/누락 음표·길이 58%) -- 이번엔 8분/16분 런(beam group)
# 밀집 구간의 "몇 개 음이 묶였는지" 세는 신호를 깨끗하게 노출. 화음은 낮춰서 방해 최소화.
set -uo pipefail
cd /workspace/round3train
MUSESCORE=/workspace/musescore/squashfs-root/AppRun
LOG=/workspace/gen_rhythm_focus.log
STATUS=/workspace/gen_rhythm_focus_status.txt
exec >> "$LOG" 2>&1
echo "=== $(date) 밀집 리듬 집중 데이터 생성 시작 ==="
: > "$STATUS"
...
POOL_DIR=data/rhythm_focus_pool
mkdir -p "$POOL_DIR"
