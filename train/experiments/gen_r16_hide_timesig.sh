#!/bin/bash
# round3train/gen_r16_hide_timesig.sh
#
# 박자표 미노출(mid-piece 촬영) 축 보강용 합성 데이터 2000장 생성. PLAN_r16_hide_timesig.md
# 참고 -- diag_new6_analysis.py(r15_cropfix_coordconv, 신규6곡) 첫 이탈 토큰 분석에서
# time이 36.8%로 1위였고, 로컬 A/B 검증(동일 콘텐츠, --hide-timesig-prob 0 vs 1.0)에서
# 박자표 안 보이면 time- 토큰 정확도가 100%->60%로 급락함을 확인함.
#
# --hide-timesig-prob 0.35(PLAN 문서의 0.3~0.4 범위 중간값, 실사 표본 실측은 아직 안 함)로
# 신규 배치 생성. r17_dense_rhythm과 동일하게 "실사 120곡 MAIN 유지 + 합성은 replay 축으로만
# 추가" 패턴을 따름(curriculum_r16_hide_timesig.sh 참고). 콘텐츠 파라미터는 r11/r17과
# 동일한 COMMON_ARGS 재사용(이번 라운드는 박자표 가시성 축만 격리).
set -uo pipefail
cd /workspace/round3train
MUSESCORE=/workspace/musescore/squashfs-root/AppRun
LOG=/workspace/gen_r16_hide_timesig.log
STATUS=/workspace/gen_r16_hide_timesig_status.txt
exec >> "$LOG" 2>&1
echo "=== $(date) 박자표 미노출 데이터 생성 시작(r16) ==="
: > "$STATUS"
  ...
  while [ "$existing" -lt "$target" ] && [ "$round" -lt 3 ]; do
    ...
    for i in $(seq 0 $((n_shards - 1))); do
      ...
      xvfb-run -a python3 generate_scores.py \
        ...
        > "/workspace/gen_r16_${stage_name}_r${round}_${i}.log" 2>&1 &
      pids+=($!)
    done
    local fail=0
...
POOL_DIR=data/r16_hide_timesig_pool
mkdir -p "$POOL_DIR"

gen_stage "timesig_single" "$POOL_DIR" 500  19000001 1900001 1
gen_stage "timesig_grand"  "$POOL_DIR" 1500 20000001 1910001 0 "${GRAND_ONLY_ARGS[@]}"

TOTAL_N=$(find "$POOL_DIR" -maxdepth 1 -name '*.png' | wc -l)
echo "=== $(date) 박자표 미노출 데이터 생성 종료: 총 ${TOTAL_N}/2000장 ==="
