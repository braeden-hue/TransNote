...
- [x] ~~더 크고 대표성 있는 held-out 셋 확보 + 정확도 재측정~~ — `realImage/exactPicture/`
      131개로 완료. Acc 82.1%(음표), 이걸 공식 FP32 베이스라인으로 확정
- [x] ~~목표 스택 결정~~ — Android(TFLite) 우선
- [x] ~~디코더 KV캐시 결정~~ — 우선 O(T²), 실기 속도 측정 후 재결정
- [x] ~~export_tflite.py 전체 파이프라인(인코더+디코더) 끝까지 실행 테스트~~ — `--segnet`을
      선택 인자로 바꿔서(현재 오선 검출엔 SegNet 안 씀) r15로 `--no_quantize` 전체 실행
      성공. 인코더 54.5MB + 디코더 131MB(둘 다 FP32 TFLite). 부산물로 FP16 변형도 이미
      생성되고 있음(27.3MB/65.7MB) — 다음 시도로 바로 가능(`_convert_tflite()`의 변형
      선택 로직만 float16으로 바꾸면 됨). `_INT8` 파일명은 지금 FP32라 오해 소지 있음,
      나중에 실제 INT8 붙일 때 네이밍 정리 필요
- [ ] 실제 모바일 기기 기준 속도 베이스라인 측정 (지금 있는 건 개발 PC CPU 수치뿐)
- [ ] INT8 캘리브레이션용 대표 이미지셋 확보 — `realImage/exactPicture/`를 그대로 쓰면 됨
      (이미 장르 다양, 대보표 위주)
- [ ] 정량 목표치(정확도 허용폭, 속도 목표) 숫자로 확정 — 이제 82.1%라는 구체적 베이스라인이
      있으니 "-Xpp 이내" 식으로 정하기 쉬워짐
- [ ] 실제 타깃 기기 확보 + 프로파일링 도구 준비
- [ ] `--quantize_decoder` 이중 입력 캘리브레이션 문제 — 시도할지 여부 결정
- [ ] (별도 트랙, 우선순위 낮음) catastrophic 실패 16곡의 공통 원인 분석 — 디코딩
      과잉생성이 주 원인인지 확인되면 양자화 전에 결정 안정성에 참고할 근거가 됨

## 실험 기록 (진행되면 추가)

| 날짜 | 정밀도 방식 | 크기 | held-out 정확도 (음표 기준) | 속도 | 메모 |
|---|---|---|---|---|---|
| 2026-08-24 | FP32 (r15, 20개 합성) | 185MB | 81.7% (Treble 85.9%/Bass 88.0%) | 개발 PC CPU ~4.9초/장(+로드 ~6.1초, 1회) | 표본 작음, 참고용 |
| 2026-08-24 | FP32 (r15, **공식 베이스라인**) | 185MB | **82.1%**(131개 실사, TER기준 80.3%) | CPU 4.7초/장(617초/131장) | 중앙값 95.8% — 분포 이분화(16곡 <50%), `train/eval_exactpicture.py`로 재현 가능 |
