...
- [ ] INT8 캘리브레이션용 대표 이미지셋 확보 — `realImage/exactPicture/`를 그대로 쓰면 됨
      (이미 장르 다양, 대보표 위주) — 단, held-out 오염 문제와 무관(캘리브레이션은 학습
      데이터 포함 여부가 문제 되지 않음)
- [ ] 정량 목표치(정확도 허용폭, 속도 목표) 숫자로 확정 — newage21~30 94.2%가 이제 구체적
      베이스라인
- [ ] 실제 타깃 기기 확보 + 프로파일링 도구 준비
- [ ] `--quantize_decoder` 이중 입력 캘리브레이션 문제 — 시도할지 여부 결정
- [ ] (별도 트랙, 우선순위 낮음) catastrophic 실패 곡들의 공통 원인 분석 — 디코딩
