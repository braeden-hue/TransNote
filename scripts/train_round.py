...
Round별 기본 경로 (--data-dir, --tokenizer, --out-dir 미지정 시 자동 설정):
    data/Round{N}/          -- 학습 이미지+JSON 쌍
    data/round_tokens/round{N}_tokens.json  -- 해당 라운드 토큰 정의
    models/round{N}/        -- 체크포인트 출력
    data/tokenizer.json     -- 공통 tokenizer (모든 라운드 공유)
"""
...
ROUND_DEFAULTS = {
    ...
    3: {"epochs_p1": 20,  "epochs_p2": 60,  "epochs_p3": 20,
        "batch_p1": 16,   "batch_p2": 8,    "batch_p3": 4,
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--data-dir",   default=None,
                   help="학습 데이터 디렉토리 (기본: data/Round{N})")
    p.add_argument("--tokenizer",  default=None,
                   help="tokenizer.json 경로 (기본: data/tokenizer.json)")
    p.add_argument("--out-dir",    default=None,
                   help="체크포인트 출력 경로 (기본: models/round{N})")
...
def main():
    ...
    print(f"    python scripts/train_round.py --round {N+1} \\")
