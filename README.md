...
> 위주로 다룬다.
>
> **2026-09-03**: 모바일 온디바이스 양자화 작업(`train/tools/export_tflite.py` 등)을
> [Model_TransNote](https://github.com/braeden-hue/Model_TransNote) 저장소로 옮겼다 —
> 학습·평가·양자화를 한 저장소에 모으는 역할 분담 정리. 상세는 그쪽 README의 "모바일
> 온디바이스 양자화" 절 참고. 이 저장소는 이제 **RunPod Serverless/Docker 프로덕션 배포만**
> 담당한다.
...
| `train/` | OMR 모델 추론 런타임 코드(PyTorch) + 체크포인트 |
...
같은 체크포인트를 모바일에서 직접 돌리는 작업(ONNX export → TFLite, 고정 크기 self-attention
KV캐시, 버킷팅 등)은 2026-09-03부터 [Model_TransNote](https://github.com/braeden-hue/Model_TransNote)
저장소로 옮겨서 진행한다 — 학습·평가·양자화를 한 저장소에 모으는 편이 자연스럽다고 판단했다.
상세 서사·벤치마크·진행 로그는 그쪽 README "모바일 온디바이스 양자화" 절과
`train/tools/QUANTIZATION_MOBILE.md` 참고.
