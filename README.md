...
|---|---|---|---|---|
| 모델 크기 | 184.6MB | 286.2MB | **116.1MB**(FP32 대비 -59%, PyTorch보다도 37% 작음) | 143.3MB(FP32 대비 50%) |
| 추론 레이턴시(평균, 2회 측정 평균) | **2.6초** | 4.2초 | **3.4초**(PyTorch 대비 1.3배) | 측정 불가(런타임 실패) |
| Peak Memory | 1.0GB | 2.4GB | 2.4GB(측정 한계, 아래 참고) | — |
| 정확도(음표 기준) | 94.2% | 93.0%(-1.2%p) | **94.3%**(PyTorch와 사실상 동일) | 측정 불가 |

(2026-08-26: cross-attention K,V 캐싱을 추가하면서 표 전체가 한 번 더 크게 개선됐다 —
"4차 시도" 참고. TFLite FP32도 함께 빨라져서(10.6초→4.2초) Hybrid와의 상대적 격차는
유지되지만 절대 수치가 모두 내려갔다.)

크기·속도·정확도 세 축 모두 FP32 TFLite보다 나은 배포 형태(Hybrid)를 확보했다. 여기까지
오는 과정에서 두 번 막혔다가 두 번 다 근본 원인을 찾아 풀었다 — 그 경로를 정직하게 남긴다.
...
가능성이 크다. 정확한 격리 측정을 하려면 백엔드별로 별도 프로세스에서 재야 한다(TODO).
