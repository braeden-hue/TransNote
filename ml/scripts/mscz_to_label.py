...
Usage:
    # 폴더 일괄 변환
    python ml/scripts/mscz_to_label.py --input-dir ml/data/test --output-dir ml/data/test

    # 단일 파일
    python ml/scripts/mscz_to_label.py --input ml/data/test/chopin_waltz.mscz

    # 파트 인덱스 지정 (0=RH, 1=LH)
    python ml/scripts/mscz_to_label.py --input-dir ml/data/test --part 0
"""
...
def _clef_tok(clef_obj) -> str:
    """music21 Clef 객체 → 토큰 문자열."""
    if clef_obj is None:
        return "clef-G"
    if isinstance(clef_obj, clef.BassClef):
        return "clef-F"
    if isinstance(clef_obj, (clef.AltoClef, clef.TenorClef)):
        return "clef-C"
    return "clef-G"


def _measure_clef_change(measure: Measure) -> str | None:
    """해당 마디에 클레프 변경 기호가 있으면 토큰 반환, 없으면 None."""
    obj = measure.getElementsByClass(clef.Clef).first()
    return _clef_tok(obj) if obj else None
...
def extract_tokens(score: Score, part_index: int | None = None) -> list[str]:
    ...
    # ── 헤더 ─────────────────────────────────────────────────────────────────
    m0 = measures[0]
    clef_obj0 = m0.recurse().getElementsByClass(clef.Clef).first() or \
                part.recurse().getElementsByClass(clef.Clef).first()
    current_clef = _clef_tok(clef_obj0)
    tokens.append(current_clef)
    ...
    for m_idx, measure in enumerate(measures):
        ...
        # 중간 클레프 변경 감지 (첫 마디는 헤더에서 이미 처리)
        if m_idx > 0:
            clef_change = _measure_clef_change(measure)
            if clef_change and clef_change != current_clef:
                tokens.append(clef_change)
                current_clef = clef_change
...
def _extract_combined(score: Score, tokens: list[str]) -> list[str]:
    """
    2파트 악보(위 오선 + 아래 오선)를 마디별로 합쳐 토큰화.

    staff-bass = 오선 위치 기반 구분자 (클레프 종류와 무관).

    마디별 구조:
        [선택: 위 오선 클레프 변경]
        [위 오선 음표]
        staff-bass
        [선택: 아래 오선 클레프 (첫 등장 또는 변경 시)]
        [아래 오선 음표]
        barline

    아래 오선이 전체 쉼표 마디인 경우 staff-bass + 음표 생략.
    """
    parts = [p.stripTies() for p in list(score.parts)[:2]]
    upper_part = parts[0]
    lower_part = parts[1]

    upper_measures = list(upper_part.getElementsByClass(Measure))
    lower_measures = list(lower_part.getElementsByClass(Measure))
    if not upper_measures:
        return tokens + ["<EOS>"]

    # ── 헤더: 위 오선 실제 클레프 기준 ──────────────────────────────────────
    m0 = upper_measures[0]
    upper_clef0 = m0.recurse().getElementsByClass(clef.Clef).first() or \
                  upper_part.recurse().getElementsByClass(clef.Clef).first()
    current_upper_clef = _clef_tok(upper_clef0)
    tokens.append(current_upper_clef)

    ks = m0.recurse().getElementsByClass(key.KeySignature).first() or \
         upper_part.recurse().getElementsByClass(key.KeySignature).first()
    tokens.append(f"key-{SHARPS_TO_KEY.get(ks.sharps, 'C')}" if ks else "key-C")

    ts = m0.recurse().getElementsByClass(meter.TimeSignature).first() or \
         upper_part.recurse().getElementsByClass(meter.TimeSignature).first()
    tokens.append(f"time-{ts.numerator}/{ts.denominator}" if ts else "time-4/4")

    # 아래 오선 초기 클레프 (첫 마디에서 읽기)
    lm0 = lower_measures[0] if lower_measures else None
    lower_clef0 = None
    if lm0:
        lower_clef0 = lm0.recurse().getElementsByClass(clef.Clef).first() or \
                      lower_part.recurse().getElementsByClass(clef.Clef).first()
    current_lower_clef = _clef_tok(lower_clef0)
    lower_clef_emitted = False   # 아래 오선 클레프 첫 등장 여부

    # ── 마디별 합치기 ─────────────────────────────────────────────────────────
    n_measures = min(len(upper_measures), len(lower_measures))
    ...
    for i in range(n_measures):
        ...
        # 위 오선 클레프 변경 감지 (첫 마디 제외)
        if i > 0:
            upper_change = _measure_clef_change(um)
            if upper_change and upper_change != current_upper_clef:
                tokens.append(upper_change)
                current_upper_clef = upper_change

        # 위 오선 음표
        tokens.extend(_measure_note_tokens(um))

        # 아래 오선 (전체 쉼표 마디 생략)
        if not _is_whole_rest(lm):
            tokens.append("staff-bass")

            # 아래 오선 클레프: 첫 등장이거나 변경 시 명시
            if not lower_clef_emitted:
                tokens.append(current_lower_clef)
                lower_clef_emitted = True
            elif i > 0:
                lower_change = _measure_clef_change(lm)
                if lower_change and lower_change != current_lower_clef:
                    tokens.append(lower_change)
                    current_lower_clef = lower_change

            tokens.extend(_measure_note_tokens(lm))

        tokens.append(_barline_token(um))
